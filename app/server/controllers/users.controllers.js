...
export default {
	login: async (req, res) => {
		try {
			...
			await searchInTables.testInTables(res, 'users', email, pass) &&
		...
		try {
			verify(req.params.token, TOKEN_SECRET, (err, decoded) => {
				...
				if (err) {
					...
					const values = ['users', 'pass', pass, 'email', email];
	...
	register: async (req, res) => {
		try {
			...
			if (verifyIfExist[0]) {
				...
				const values = ['users', 'full_name', 'email', 'pass', 'birthdate', 'gender', full_name, email, password, birthdate, gender];
